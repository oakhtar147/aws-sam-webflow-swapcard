...
async function createNewExhibitors(
	...
	for (const swapcardSpeaker of SWAPCARD_MEMBERS_FOR_EXHIBITION) {
	...
	for (const ex of EXHIBITORS_IN_SWAPCARD_SESSIONS[0]) {
				...
				const newExhibitor = await webflow.createItem(
					{
						...
						fields: {
							_archived: false,
							_draft: false,
							slug: ex.name.trim().toLowerCase().replace(" ", "-") + makeid(5),
							"swapcard-id": ex.id,
							name: ex.name,
