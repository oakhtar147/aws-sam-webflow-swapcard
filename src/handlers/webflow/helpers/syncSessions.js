...
async function syncSessions(
  ...
  const idMap = SESSIONS_IN_WEBFLOW_COLLECTION.reduce(
  ...
  for (const session of SWAPCARD_SESSIONS) {
    ...
    if (!swapCardIdsInWebflow.includes(session.id)) {
      ...
      try {
        const sessionToBeUpdated = SESSIONS_IN_WEBFLOW_COLLECTION.find(
          (s) => s["swapcard-id"] === session.id
        );

        if (generateHash(session) !== sessionToBeUpdated.hash) {
          await webflowRateLimiterWorkaround({
            webflow,
            collectionId: COLLECTION_ID.sessions,
            callback: async () =>
